    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if(sessao.getAttribute("usuarioAutenticado") != null){
            try{
                ...
                setor.setId(setorDAO.proxID());                
                setor.setNome(String.valueOf(request.getParameter("txtNome")).toUpperCase());
                setor.setAbreviatura(String.valueOf(request.getParameter("txtAbreviatura")).toUpperCase());
                setor.setIdDivisaoSecao(Integer.parseInt(request.getParameter("txtDivisaoSecao")));
                
                setorDAO.insert(setor);

            }catch(Exception ex){
                //e=2: erro durante realização do cadastro
                response.sendRedirect("/controlevisitantes/restrito/setor/setor.jsp?e=2");
                throw new ServletException(ex);
            }
            //e=1: cadastro sucesso
            response.sendRedirect("/controlevisitantes/restrito/setor/setor.jsp?e=1");
        }
        else{
